...
max = (3..16).pairs.map do |pair|
  x = pair[0]
  y = pair[1]
  [(0..3).map { |i| matrix[y][x+i].to_i }.inject(:*), #right
    (0..3).map { |i| matrix[y][x-i].to_i }.inject(:*), #left
    (0..3).map { |i| matrix[y+i][x].to_i }.inject(:*), #down
    (0..3).map { |i| matrix[y-i][x].to_i }.inject(:*), #up
